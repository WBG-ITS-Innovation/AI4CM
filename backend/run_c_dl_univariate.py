...
def main() -> None:
    _log("===== Georgia C·DL (univariate) runner =====")
    env = _get_env()
    ...
    out_root = env["TG_OUT_ROOT"] or "./outputs/dl_univariate"
    ...
    # If user specifies just one model via TG_MODEL_FILTER => set as univariate-only
    models = _models_from_filter(env["TG_MODEL_FILTER"])
    cfg.models_univariate = models

    # Optional multivariate exogenous cap (unused in univariate, but harmless)
