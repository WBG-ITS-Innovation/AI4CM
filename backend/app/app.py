...
def _empty_runs_table():
    """Provide runs-table in the initial layout so callbacks never see a missing ID."""
    ...
    return dash_table.DataTable(data=[], columns=columns, page_size=8, id="runs-table",
...
app.layout = dbc.Container(fluid=True, children=[
    dbc.Row([
        dbc.Col([
            ...
            html.Div(id="runs-div", children=[_empty_runs_table()]),
...
def select_run(selected, data):
    if not selected or not data: return no_update
    r = data[selected[0]]
...
def render_forecast(run_id):
    ...
    ds = [d for d in db.list_datasets() if d[0]==ds_id]
    if not ds:
        return html.Div("Dataset not found.")
    try:
        df = load_dataframe(ds[0][2])
    except Exception as e:
        return html.Div(f"Failed to load data: {e}")
    fig = go.Figure()
