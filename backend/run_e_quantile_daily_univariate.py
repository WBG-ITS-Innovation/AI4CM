...
def main():
    t0 = time.time()
    print("[runner] ===== Georgia E·Quantile (univariate) runner =====")
    cfg = Config(
        ...
        windows_daily=tuple(json.loads(_getenv("TG_PARAM_OVERRIDES", "{}")).get("windows_daily", [5,20])),
        out_root=_getenv("TG_OUT_ROOT", "outputs"),
        demo_clip_months=json.loads(_getenv("TG_PARAM_OVERRIDES", "{}")).get("demo_clip_months", None),
        variant="univariate"
    )
